package com.unity3d.ads.android.data;

import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import com.unity3d.ads.android.UnityAdsDeviceLog;
import java.lang.reflect.Method;

public class UnityAdsGraphicsBundle {
   public static String ICON_AUDIO_MUTED_32x32 = "";
   public static String ICON_AUDIO_MUTED_50x50 = "";
   public static String ICON_AUDIO_UNMUTED_32x32 = "";
   public static String ICON_AUDIO_UNMUTED_50x50 = "";

   public static Bitmap getBitmapFromString(String var0) {
      Bitmap var1 = null;
      if (var0 != null) {
         Method var3;
         label37: {
            label36: {
               boolean var10001;
               Class var7;
               try {
                  var7 = Class.forName("android.util.Base64");
               } catch (Exception var13) {
                  var10001 = false;
                  break label36;
               }

               if (var7 != null) {
                  label29: {
                     Method var9;
                     try {
                        Class[] var8 = new Class[]{String.class, Integer.TYPE};
                        var9 = var7.getMethod("decode", var8);
                     } catch (Exception var11) {
                        var10001 = false;
                        break label29;
                     }

                     var3 = var9;
                  }
               } else {
                  label32: {
                     try {
                        UnityAdsDeviceLog.error("Could not find base64 decode class");
                     } catch (Exception var12) {
                        var10001 = false;
                        break label32;
                     }

                     var3 = null;
                  }
               }
               break label37;
            }

            UnityAdsDeviceLog.error("Proper decode method could not be found, disabling mute-button feature");
            var3 = null;
         }

         byte[] var5;
         try {
            Object[] var6 = new Object[]{var0, 0};
            var5 = (byte[])((byte[])var3.invoke((Object)null, var6));
         } catch (Exception var10) {
            UnityAdsDeviceLog.error("Problems invoking decode method");
            var5 = null;
         }

         var1 = BitmapFactory.decodeByteArray(var5, 0, var5.length);
      }

      return var1;
   }
}
